...
async def generate_team_id(event_id: str, db: AsyncSession) -> str:
    type_res = await db.execute(select(Event).where(Event.eid == event_id))
    event = type_res.scalar_one_or_none()
    if not event:
        raise HTTPException(status_code=400, detail="Invalid event ID")

    prefix = 'AI' if event.event_type == 'Software' else 'IE'
...
async def generate_food_coupon_id(db: AsyncSession):
    result = await db.execute(select(func.count(Food.coupon_id)))
    count = result.scalar() or 0
    return f"F{count + 1:03}"
...
async def submit_team_register(request: Request, db: AsyncSession = Depends(get_sql_session)):
    ...
    # ❌ REMOVE THIS LINE - Don't get a single food_preference for the whole team
    ...
    try:
        ...
        for m in existing_members:
            ...
            # ✅ GET INDIVIDUAL FOOD PREFERENCE FROM EACH MEMBER
            member_food_preference = m.get("food_preference", "veg")  # Default to veg if not provided
            ...
            # ✅ CREATE FOOD COUPON WITH THIS MEMBER'S PREFERENCE
            coupon_id = await generate_food_coupon_id(db)
            food_coupon = Food(
                coupon_id=coupon_id,
                pid=pid,
                food_preference=member_food_preference,  # ✅ Use member's individual preference
                flag=0,
